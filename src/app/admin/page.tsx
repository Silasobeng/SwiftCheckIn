...
export default function AdminPage() {
  ...
  const regenerateKioskCode = async () => {
    if (!window.confirm('Generate a new kiosk code? The current one stops working immediately.')) return;
    setRegeneratingCode(true); setMessage(null); setError(null);
    try {
      const res = await fetch('/api/settings',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({regenerate_kiosk_code:true})});
      const data = await res.json();
      if (!res.ok) { setError(data.error||'Could not generate a new code.'); return; }
      setMessage('New kiosk code generated.');
      loadData();
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-7 sm:py-9 space-y-5">
        ...
        {tab==='settings' && (
          <div className="space-y-5 animate-fade-in">
            ...
            {/* Kiosk access code — previously generated by the database but
                never shown anywhere, so nobody could unlock their own tablet. */}
            <div className="card">
              <div className="panel-label mb-1" style={{display:'block'}}>Kiosk access code</div>
              <p style={{fontSize:13,color:'#7A6E60',fontWeight:300,marginBottom:16,maxWidth:520,lineHeight:1.7}}>
                Type this on the tablet to unlock check-in and get back to normal.
                Tap the padlock in the bottom-right corner of the kiosk screen.
                Keep it with your ushers, not on the tablet.
              </p>
              <div className="flex flex-wrap items-center gap-3">
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:30,letterSpacing:'0.3em',color:'#16243A',background:'#F8F4EE',border:'1px solid #E4DFD5',borderRadius:12,padding:'12px 20px 12px 26px',lineHeight:1.2}}>
                  {settings?.kiosk_access_code || '——————'}
                </div>
                <button onClick={regenerateKioskCode} disabled={regeneratingCode} className="btn btn-secondary text-sm">
                  {regeneratingCode ? 'Generating…' : 'Generate new code'}
                </button>
              </div>
            </div>
